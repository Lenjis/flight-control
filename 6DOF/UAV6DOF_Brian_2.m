function [sys, x0, str, ts] = UAV6DOF_Brian_2(t, x, u, flag)
    %
...
function [sys,x0,str,ts]=mdlInitializeSizes
    ...
    % Using the trim data
    Vt = 48.0652941657053;
    alpha_deg = -0.657088117287537; %配平迎角
    beta_deg = 1;
    theta = alpha_deg;
    psi_hmr = beta_deg;
    H = 1000; 
    x0 = [Vt; deg2rad(alpha_deg); deg2rad(beta_deg); 0; 0; H; 0; 0; 0; deg2rad(0); deg2rad(theta); deg2rad(psi_hmr)];
...
function sys=mdlDerivatives(t,x,u)
    % ---状态量-------
    Vt    = x(1); PN = x(4); P = x(7); phi   = x(10); %#ok<*NASGU>
    ...
    eng = u(4); % thruttle (0-100)

    % ---质量数据-------
    mass = 17;
    Ix = 1.71; Iy = 5.54; Iz = 4.15;
    cbar = 0.423;
    SA = 1.3536;
    b = 3.2;
    g = 9.81;
    ...
    % ---- 计算空气密度和动压----
    [ru, mach] = UAV_density(H, Vt); %#ok<*ASGLU> % [air density] [mach number]
    ...
    % ---- 计算气动力矩----
    [CR0, CR_R, CR_P, CR_rud, CR_ail] = UAV_CR(beta); %-----滚转
    CR = CR0 + b / (2 * Vt) * (CR_R * R + CR_P * P) + CR_ail * ail + CR_rud * rud;
    Lbar = qs * b * CR;
    [CM_alpha, CM_ele, CM_Q, CM_dalpha] = UAV_CM(alpha); %-----俯仰
    CM = CM_alpha + CM_ele * ele + cbar / (2 * Vt) * (CM_Q * Q);
    M = qs * cbar * CM;
    [CN0, CN_R, CN_P, CN_rud, CN_ail] = UAV_CN(beta); %-----偏航
...
function sys=mdlOutputs(t,x,u)
    ...
    %-------输出量--------------
    sys = [
            Vt;           rad2deg(alpha); rad2deg(beta);
            PN;           PE;             H;
            rad2deg(P);   rad2deg(Q);     rad2deg(R);
...
function [CL0, CL_ele] = UAV_CL(alpha)
    ...
    %CL0 = interp1(IDX_alpha, TBL_CL0, alpha_deg, "spline");
    CL_ele = 0.00636;
%end UAV_CL
...
function [CD0] = UAV_CD(alpha)
    ...
    %CD0 = interp1(IDX_alpha, TBL_CD0, alpha_deg, "spline");
%end UAV_CD
...
function [CM_alpha, CM_ele, CM_Q, CM_dalpha] = UAV_CM(alpha)
    ...
    %CM_alpha = interp1(IDX_alpha, TBL_CM0, alpha_deg, "spline");
    CM_ele = -0.02052;
    CM_Q = -9.3136;
    CM_dalpha = -1.0354;
%end UAV_CM
