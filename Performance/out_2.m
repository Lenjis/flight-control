...
%IDX_alpha = [-4.00; -2.00; 0.00; 2.00; 4.00; 6.00; 8.00; 10.00; 12.00; 14.00];
IDX_alpha = [-4; -2; 0; 2; 4; 8; 12; 16; 20];
%TBL_CL0 = [-0.12006; 0.02319; 0.16666; 0.30218; 0.45146; 0.59858; 0.74889; 0.87109; 0.97428; 1.07922];
TBL_CL0 = [-0.219; -0.04; 0.139; 0.299; 0.455; 0.766; 1.083; 1.409; 1.743];
%TBL_CD0 = [0.03564; 0.03154; 0.03319; 0.03586; 0.04214; 0.05142; 0.06133; 0.08265; 0.11291; 0.13918];
TBL_CD0 = [0.026; 0.024; 0.024; 0.028; 0.036; 0.061; 0.102; 0.141; 0.173];
TBL_Cm = [0.03844; 0.02496; 0.00820; -0.01545; -0.04472; -0.07026; -0.09188; -0.17399; -0.26750; -0.31096];
TBL_Mz_ele = [0.00562; 0.00555; 0.00579; 0.00617; 0.00684; 0.00709; 0.00616; 0.01141; 0.01168; 0.00638];

figure(1);
plot(IDX_alpha, TBL_CL0);
title("升力系数");
xlabel("AoA/deg");
ylabel("C_L");
grid on;
...
figure(3);
plot(IDX_alpha, TBL_CL0 ./ TBL_CD0);
title("升阻比");
xlabel("AoA/deg");
ylabel("C_L/C_D");
grid on;
...
figure(6);
H = 0:1000:6000;
T = zeros(length(H), length(V));
for h = H
    i = h / 1000 + 1;
    plot(V, T_req(i, :)); %绘制平飞需用推力
    hold on;
end
for h = H
    i = h / 1000 + 1;
    T(i, :) = UAV_thrust(i, V) * g; %绘制实际推力曲线
    plot(V, T(i, :), "LineStyle", "--");
    hold on;
    [x_intersect, y_intersect] = polyxpoly(V, T_req(i, :), V, T(i, :)); %计算交点并保存
    plot(x_intersect, y_intersect, 'o');
    hold on;
    Vmax(i) = x_intersect;
end
legend('H=0m', 'H=1000m', 'H=2000m', 'H=3000m', 'H=4000m', 'H=5000m', 'H=6000m', 'Location', 'northwest');
